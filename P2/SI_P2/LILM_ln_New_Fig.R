...
time<-rep(1:t_end)
plot(time, newinf_true2, xlim=c(min(time), max(time)), ylim=c(0,350),ylab="No. of new infections",main="(b) Conditional logistic ILM", xlab="Time", type="b", pch=20, lwd=2,xaxt="n",col="red")
axis(1, at=1:max(time))
par(new=TRUE)
for(i in 1:500){
  lines(time, yatg[i,], col="green",lwd=0.2)
}
